...
class StudentProfileController extends Controller
{
    ...
    {
        $id = Auth::user()->id;
		$adminData = Students::find($id);
        return view('students.profile.profile_show',compact('adminData'));
    ...
    public function profileUpdate(Request $request)
    {
        ...
        $updateData->telephone2 = $request->telephone2;
  ...
  if ($request->file('profile_photo_path')) {
    ...
    @unlink(public_path('upload/student_images/'.$updateData->profile_photo_path));
    $image = $request->file('profile_photo_path');
    $name_gen = $image->hashName();
    $new_img = $manager->read($request->file('profile_photo_path'));
    $new_img = $new_img->resize(102,102);
    $new_img->toJpeg(80)->save(base_path('public/upload/student_images/'.$name_gen));
    $save_url = $name_gen;
