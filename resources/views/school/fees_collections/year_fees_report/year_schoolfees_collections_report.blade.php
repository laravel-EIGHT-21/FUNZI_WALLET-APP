...
<head>
    ...
    <title>Yearly SchoolFees Collection | Report Print</title>
...
<br/>
            ...
            <div class="row">
              <div class="col-12">
                ...
                <h5><u>YEARLY SCHOOLFEES COLLECTIONS REPORT FOR YEAR : {{$schoolfees['0']['year']}}</u></h5>
              </div>
            </div>
                ...
                @foreach($schoolfees as $key => $value )
                  ...
                  @php
      $term= App\Models\students_schoolfees_records::where('term_id',$value->term_id)->orderBy('id','desc')->get();

	  $total_amount_paid = App\Models\students_schoolfees_records::where('term_id',$value->term_id)->where('year',$value->year)->sum('amount');
    
    
	  $total_amount = App\Models\students_schoolfees_records::where('term_id',$value->term_id)->where('year',$value->year)->sum('total_amount');

    $total_bal = (float)$total_amount-(float)$total_amount_paid;

    @endphp

<td> {{ $term['0']['term']['name'] }}</td>
<td>ugx {{ ($total_amount_paid) }}</td>	
<td>ugx {{ ($total_amount) }}</td>	
                      ...
                      <div class="col-md-6 d-flex justify-content-md-end mt-2">
                        <div class="invoice-calculations">
                          <div class="d-flex justify-content-between mb-2">
                            <span class="w-px-150">Total Fees Paid:</span>
                            <h6 class="mb-0 pt-1">ugx {{$total_fees_paid}}</h6>
                          </div>
                          <div class="d-flex justify-content-between mb-2">
                            <span class="w-px-150">Total Amount:</span>
                            <h6 class="mb-0 pt-1">ugx {{$total_fees_amount}}</h6>
                          </div>
            
            
            @php 
            
            $total_sum_bal = (float)$total_fees_amount-(float)$total_fees_paid;
