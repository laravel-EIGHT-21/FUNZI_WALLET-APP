...
<head>
    ...
    <title>Yearly School Income Statement | Report Print</title>
...
<br/>
            ...
            <div class="row">
              <div class="col-12">
                ...
                <h5><u>YEARLY INCOME STATEMENT REPORT FOR YEAR : {{$schoolfees['0']['year']}}</u></h5>
              </div>
            </div>
...
<br/>
      ...
      $term= App\Models\students_schoolfees_records::where('term_id',$value->term_id)->orderBy('id','asc')->get();
...
<td>ugx {{ ($total_amount_paid) }}</td>	
            ...
            $expense_paid = App\Models\expenses::where('category_id',$exp->category_id)->where('year',$exp->year)->sum('amount');
          
            @endphp
            	
              <td> {{ $exp['category']['name'] }}</td>	
              <td>ugx {{ ($expense_paid) }}</td>	
                  ...
                  <table class="table border table-bordered text-nowrap">
                      <thead class="border-top">
                        ...
                        $net_income = (float)$total_fees_paid-(float)$total_expense_paid;
